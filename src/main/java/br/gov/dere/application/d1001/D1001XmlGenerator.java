...
public class D1001XmlGenerator {
  ...
  public String generate(D1001 d) throws Exception {
    ...
    var contrib=child(doc,evt,"ideContrib"); text(doc,contrib,"nrInsc",d.cnpjRoot()); var period=child(doc,evt,"idePeriodo"); text(doc,period,"iniValid",d.validFrom()); if(d.validTo()!=null)text(doc,period,"fimValid",d.validTo()); if(d.operation()!=3&&d.newValidFrom()!=null){var nv=child(doc,period,"novaValidade");text(doc,nv,"iniValid",d.newValidFrom());if(d.newValidTo()!=null)text(doc,nv,"fimValid",d.newValidTo());}
    if(d.operation()!=3){var info=child(doc,evt,"infoContrib"); text(doc,info,"regTribPrinc",d.primaryTaxRegime()); for(var v:d.secondaryTaxRegimes())text(doc,info,"regTribSecund",v); text(doc,info,"indNatTrib",d.taxNature());
